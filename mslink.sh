...
# The final backslash is removed if there is one
...
TARGET_ROOT=${TARGET_ROOT}$(for i in `seq 1 21`;do echo -n '\x00';done) # Needed from Vista and higher otherwise the link is considered empty (I have not found any information about this)
...
echo "Creating a type shortcut \""${TYPE_TARGET}" "${TYPE_LNK}"\" with target "${LNK_TARGET} 1>&2
